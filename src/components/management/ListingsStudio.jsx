      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-3xl">
              <div className="text-xs font-semibold uppercase tracking-[0.25em] text-white/55">
                Management Upload Studio
              </div>
              <h2 className="mt-2 font-display text-[28px] font-bold leading-tight">Listings workspace</h2>
              <p className="mt-2 text-sm leading-relaxed text-white/70">
                Create, edit, and manage all staycation properties in one place. Changes publish to the public catalog
                immediately.
              </p>
            </div>
            <div className="flex flex-wrap gap-2.5">
              <button
                ...
                className="rounded-xl bg-white px-5 py-2.5 text-sm font-semibold text-[#0F1F3D]"
              >
                + New Listing
              </button>
              <button
                ...
                className="rounded-xl border border-white/20 px-5 py-2.5 text-sm font-medium text-white/80"
              >
                Back to Dashboard
              </button>
            </div>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-500">
                The cramped in-page overlay is gone. Bulk upload now has full-width breathing room with property
                selection, asset targeting, and clearer file mapping rules.
              </p>
            </div>
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
            <div className="rounded-3xl bg-gradient-to-br from-[#1a2f52] to-[#0F1F3D] p-5 text-white">
              ...
              <p className="mt-3 text-xs leading-relaxed text-white/60">
                Upload 1 file to reuse for every selected property, or upload{' '}
                {studio.selectedBulkListings.length || 'matching'} files to map in property order.
              </p>
            </div>
            <div className="rounded-3xl border border-ice-200 bg-ice-50 p-5">
              <div className="mb-4 flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-lg font-semibold text-brand-950">
                    Choose the listings that should receive the upload
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {studio.availableListings.map((listing) => (
                  <label
                    key={listing.id}
                    className="flex cursor-pointer items-start gap-3 rounded-2xl border border-ice-200 bg-white px-4 py-4 shadow-sm"
                  >
                    <input
                      type="checkbox"
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="mt-1 block text-sm text-slate-500">
                        {listing.location || 'Add a location to complete this card.'}
                      </span>
                    </span>
                  </label>
                ))}
        ...
        <form onSubmit={studio.handleListingSubmit} className="space-y-6">
          ...
          >
            <div className="grid gap-4 md:grid-cols-[1.1fr_0.9fr]">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    studio.setSelectedListingId(e.target.value);
              ...
              <div className="flex flex-wrap gap-3">
                <button
                  type="button"
                  onClick={studio.handleCreateListing}
                  className="btn-primary flex-1 px-5 py-3 text-sm"
                >
                  <span>+ Add New Listing</span>
                </button>
                <button
                  type="button"
                  onClick={studio.handleDeleteListing}
                  className="flex-1 rounded-2xl border border-rose-200 px-5 py-3 text-sm font-semibold text-rose-600"
                >
            ...
            {studio.showDeleteConfirm ? (
              <div className="mt-3 flex flex-wrap items-center gap-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-medium text-red-800">
                <span>
                  Are you sure you want to delete <strong>{studio.selectedListing?.name}</strong>? This cannot be
                  undone.
                </span>
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={studio.confirmDelete}
                    className="rounded-lg bg-red-600 px-3.5 py-1.5 text-xs font-semibold text-white"
                  >
                    Yes, Delete
                  </button>
                  <button
                    type="button"
                    onClick={studio.cancelDelete}
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  id="location"
                  name="location"
                  value={studio.listingForm.location}
                  onChange={studio.handleListingFieldChange}
                  className="form-input"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  type="number"
                  min="0"
                  value={studio.listingForm.price}
                  onChange={studio.handleListingFieldChange}
                  className="form-input"
                />
              </div>
              <div>
                ...
                <input
                  id="statusNote"
                  name="statusNote"
                  value={studio.listingForm.statusNote}
                  onChange={studio.handleListingFieldChange}
                  className="form-input"
              ...
              <div>
                ...
                <select
                  id="publishStatus"
                  name="publishStatus"
                  value={studio.listingForm.publishStatus}
                  onChange={studio.handleListingFieldChange}
                  className="form-input"
                >
                    ...
                    <div key={field} className="rounded-3xl border border-ice-200 bg-slate-50 p-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0 flex-1">
                          ...
                          <input
                            id={field}
                            name={field}
                            value={studio.listingForm[field] || ''}
                            onChange={studio.handleListingFieldChange}
                            className="form-input"
                      ...
                      >
                        <input
                          ...
                          onChange={(e) => studio.handleMediaUpload(field, e.target.files?.[0])}
                          className="hidden"
                        />
                      ...
                      <div className="mt-4 rounded-2xl border border-ice-200 bg-white p-3">
                        ...
                        {isVideoField ? (
                          ...
                          <img
                            src={preview || currentUrl || studio.selectedListing?.image}
                            alt={`${studio.listingForm.name} ${config.label.toLowerCase()}`}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <textarea
                  ...
                  name="bestFor"
                  rows="4"
                  value={studio.listingForm.bestFor}
                  onChange={studio.handleListingFieldChange}
                  className="form-input"
          ...
          <div className="rounded-3xl bg-ice-50 p-4 text-sm text-slate-500">
            Changes here update the public staycation cards and the booking flow because management is now the source of
            truth for listing content.
          ...
          <button
            type="submit"
            disabled={studio.isSavingListing}
        ...
        <section className="rounded-[1.8rem] border border-ice-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-2.5 overflow-hidden rounded-xl bg-[#1E3560] h-24 flex items-center justify-center">
            {studio.selectedListing?.image ? (
              <img
                src={studio.selectedListing.image}
                alt={studio.selectedListing.name}
                className="h-full w-full object-cover"
              />
            ) : (
              <div className="text-[28px] font-bold text-white/30">
          ...
          <div className="mt-1 text-sm text-slate-500">
            {studio.selectedListing?.location || 'Location still needed'}
          </div>
          <div className="mt-4 flex items-center justify-between rounded-2xl bg-ice-50 px-4 py-3">
            ...
            <span className="text-lg font-bold text-brand-700">
              {formatCurrency(Number(studio.listingForm.price || 0))}
            </span>
          </div>
          <div className="mt-4 space-y-3">
            {STUDIO_SECTIONS.map((section) => (
              <button
                ...
                onClick={() =>
                  document
                    .getElementById(`studio-${section.id}`)
        ...
        <section className="rounded-[1.8rem] border border-ice-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            <div className="flex items-center justify-between rounded-xl bg-ice-50 px-3 py-2.5">
              ...
              <span
                className={`text-xs font-semibold rounded-full px-2 py-0.5 ${studio.listingForm.publishStatus === 'published' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}
              >
          ...
          <div className="mt-5 space-y-4">
            {[
              ...
              },
              {
                title: 'Videos',
                description:
                  'Attach walkthrough or short promo videos so the client can preview the staycation clearly.',
              },
              {
                title: 'Facilities',
