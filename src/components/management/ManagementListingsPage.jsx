...
export function ManagementListingsPage({
  listings = FEATURED_PROPERTIES, bookings = [], revenue = 0,
  ownerApplications = [], reviewSubmissions = [],
  onSaveListing, onDeleteListing, onShowPage, onSignOut, authUser, formatCurrency,
}) {
    ...
    <div className="flex h-screen overflow-hidden bg-[#F5F7FA] text-sm text-[#0F1F3D]" style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      ...
      <aside className="flex h-full w-[220px] min-w-[220px] flex-shrink-0 flex-col bg-[#0F1F3D]">
        ...
        <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto px-2.5 py-3.5">
          <div className="px-2 pb-1 pt-2.5 text-[10px] font-medium uppercase tracking-[0.06em] text-white/35">Overview</div>
          {[{ id: 'dashboard', label: 'Dashboard', onClick: () => onShowPage('dashboard') }, { id: 'bookings', label: 'Bookings', onClick: () => onShowPage('dashboard') }, { id: 'analytics', label: 'Analytics', onClick: () => onShowPage('dashboard') }].map((item) => (
          ...
          <div className="px-2 pb-1 pt-2.5 text-[10px] font-medium uppercase tracking-[0.06em] text-white/35">Listings</div>
          {[{ id: 'manage-listings', label: 'Manage listings' }, { id: 'upload-studio', label: 'Upload studio' }].map((item) => (
          ...
          <div className="px-2 pb-1 pt-2.5 text-[10px] font-medium uppercase tracking-[0.06em] text-white/35">Clients</div>
          {[{ id: 'owner-leads', label: 'Owner leads', onClick: () => onShowPage('dashboard') }, { id: 'evaluate-leads', label: 'Evaluate leads', onClick: () => onShowPage('dashboard') }, { id: 'email-activity', label: 'Email activity', onClick: () => onShowPage('dashboard') }].map((item) => (
            <button key={item.id} type="button" onClick={item.onClick}
              className="flex w-full items-center gap-2.5 rounded-md border-none px-2.5 py-2 text-left text-[13px] text-white/55"
            >
              <svg viewBox="0 0 24 24" className="h-4 w-4 flex-shrink-0" style={{ stroke: 'currentColor', fill: 'none', strokeWidth: 1.8, strokeLinecap: 'round', strokeLinejoin: 'round' }}>
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden" style={{ height: '100%' }}>
        <div className="flex h-[52px] flex-shrink-0 items-center gap-3.5 border-b border-[rgba(15,31,61,0.12)] bg-white px-6">
          ...
          <div className="ml-auto flex items-center gap-2.5">
            ...
            <button type="button" onClick={() => onShowPage('dashboard')} className="btn-primary flex items-center gap-1.5 rounded-md border-none bg-[#0F1F3D] px-3.5 py-1.5 text-xs font-medium text-white">
        ...
        <div className="flex flex-1 flex-col gap-[18px] overflow-y-auto p-6">
          <div className="flex items-stretch gap-0 rounded-xl bg-[#0F1F3D] px-6 py-[18px]">
            <div className="flex flex-col gap-0.5 pr-6">
              ...
              <span className="text-[11px] text-white/55">{draftListingsCount ? `${draftListingsCount} draft in progress` : 'All active listings are published'}</span>
            </div>
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-[22px] font-semibold text-white">{ownerApplications.length + reviewSubmissions.length}</span>
              <span className="text-[11px] text-white/55">Owner and evaluation demand connected to this workspace</span>
            </div>
            <div className="mx-6 w-px self-stretch bg-white/10 flex-shrink-0" />
            <div className="flex items-center flex-1 pl-6 text-xs leading-relaxed text-white/55">The listing studio is the source of truth for the public staycation catalog.</div>
          </div>

          <div className="rounded-xl border border-[rgba(15,31,61,0.12)] bg-white p-[18px]">
            <ListingsStudio listings={listings} onSaveListing={onSaveListing} onDeleteListing={onDeleteListing} onShowPage={onShowPage} formatCurrency={formatCurrency} />
          </div>
          ...
          <div className="rounded-xl border border-[rgba(15,31,61,0.12)] bg-white p-[18px]">
            <div className="mb-3.5 flex items-center justify-between">
              ...
              <button type="button" onClick={() => onShowPage('dashboard')} className="border-none bg-transparent text-xs font-medium text-[#0F1F3D]">Back to dashboard</button>
            </div>
