...
export async function submitSupportRequest(request) {
  ...
  store.dashboardEmails = [{ title: 'New Support Request', detail: `${request.topic} · ${request.email || 'guest message'}`, tone: 'indigo' }, ...store.dashboardEmails].slice(0, MAX_DASHBOARD_PREVIEW_ITEMS);
  saveStore(store);
