...
import { getAuthenticatedUser, upsertRemote, fetchRemoteManagementListings, fetchRemoteBookingTransactions, uploadListingMediaFile } from './supabaseClient';
...
function normalizeListingPayload(listing) {
  ...
  const blockedDates = Array.isArray(listing.blockedDates) ? listing.blockedDates
...
function mergeManagementListings(listings = []) {
  ...
  const mergedDefaults = FEATURED_PROPERTIES.map((l) => normalizeListingPayload({ ...l, ...(listingMap.get(l.id) || {}) })).filter((l) => !l.isDeleted);
  const extras = listings.filter((l) => !FEATURED_PROPERTIES.some((d) => d.id === l.id) && !l.isDeleted).map((l) => normalizeListingPayload(l));
...
function fromRemoteManagementListing(record) {
  ...
  return normalizeListingPayload({
    ...
    name: record.name ?? defaults.name, location: record.location ?? defaults.location,
    ...
    summaryImage: record.summary_image ?? defaults.summaryImage, thumbnail: record.thumbnail ?? defaults.thumbnail,
...
function toRemoteManagementListing(listing) {
  return {
    id: listing.id, name: listing.name, location: listing.location, price: Number(listing.price) || 0,
    rating_label: listing.ratingLabel || '5.0', review_count: Number(listing.reviewCount) || 0,
    badge: listing.badge || 'Featured Stay', badge_icon: listing.badgeIcon || 'star',
    status_note: listing.statusNote || '', mood: listing.mood || '', best_for: listing.bestFor || '',
    image: listing.image || '', summary_image: listing.summaryImage || '', thumbnail: listing.thumbnail || '',
    video_url: listing.videoUrl || '', schedule: listing.schedule || '',
    publish_status: listing.publishStatus || 'published', availability_notes: listing.availabilityNotes || '',
    blocked_dates: Array.isArray(listing.blockedDates) ? listing.blockedDates : [],
    is_deleted: Boolean(listing.isDeleted), amenities: Array.isArray(listing.amenities) ? listing.amenities : [],
    updated_by: listing.updatedBy || null, updated_at: new Date().toISOString(),
  };
}
...
export async function saveManagementListing(listingInput) {
  ...
  const uploadResults = await Promise.all(uploadFields.map(([fn]) => uploadListingMediaFile(listing.id, fn, mediaFiles[fn])));
  ...
  store.dashboardEmails = [{ title: 'Listing Updated — Management', detail: `Saved for ${savedListing.name}`, tone: 'accent' }, ...store.dashboardEmails].slice(0, MAX_DASHBOARD_PREVIEW_ITEMS);
  saveStore(store);
  return { store, remote: { saved: remote.saved, error: remote.error, uploadedMediaCount: uploadResults.filter((r) => r?.uploaded).length, attemptedMediaCount: uploadResults.length } };
}
...
export async function deleteManagementListing(listingId) {
  ...
  const remote = await upsertRemote('management_listings', toRemoteManagementListing({ ...existing, isDeleted: true, updatedBy: currentUser?.id || null }));
  store.managementListings = mergeManagementListings(store.managementListings.map((l) => l.id === listingId ? { ...l, isDeleted: true } : l).filter((l) => l.id !== listingId || FEATURED_PROPERTIES.some((d) => d.id === listingId)));
  store.dashboardEmails = [{ title: 'Listing Deleted — Management', detail: `Removed ${existing.name}`, tone: 'accent' }, ...store.dashboardEmails].slice(0, MAX_DASHBOARD_PREVIEW_ITEMS);
  saveStore(store);
  return { store, remote };
}
...
export async function syncRemoteData(options = {}) {
  ...
  if (includeListings && remoteListings.saved && remoteListings.listings.length) store.managementListings = remoteListings.listings;
  if (includeBookings && remoteBookings.saved) {
    ...
    store.dashboardBookings = remoteBookings.transactions.slice(0, MAX_DASHBOARD_PREVIEW_ITEMS).map((tx) => ({
      ...
      status: String(tx.bookingStatus || 'confirmed').replace(/[-_]/g, ' ').replace(/^\w/, (m) => m.toUpperCase()),
      image: `https://picsum.photos/seed/${encodeURIComponent(tx.bookingForm.guestName || 'Guest')}/40/40.jpg`,
    }));
    store.dashboardRevenue = remoteBookings.transactions.reduce((t, tx) => t + (tx.paymentStatus === 'refunded' ? 0 : Number(tx.bookingSummary.total) || 0), 0);
  }
  saveStore(store);
