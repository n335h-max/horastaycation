...
export function ownerLeadTemplate(input = {}) {
  const { ownerName, ownerEmail, ownerAddress, unitCount, budget } = sanitizeTemplateData(input);
...
export function evaluationRequestTemplate(input = {}) {
  const { evaluatorName, evaluatorEmail, evaluatorAddress, unitCount } = sanitizeTemplateData(input);
